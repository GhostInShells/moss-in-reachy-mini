...
MEMORY = os.getenv("REACHY_MINI_MEMORY", "desktop_memory")
...
async def build_main_agent(parent: Container) -> MainAgent:
    ...
    instructions = load_instructions(
        container,
        files=["desktop/memory_rules.md","desktop/system_rules.md",
               "ctml_enrich.md", "websearch.md", "news.md", "speech.md"],
        storage_name="instructions",
    )
...
async def main():
    ...
    with workspace_container(ws_dir) as container:
        logger = setup_logger(
            str(ws_dir.joinpath("runtime/logs/moss_desktop.log").absolute()),
        )
