...
class MusicSearch:
    ...
    async def _push_choreography_event(self, title: str, artist: str, info: dict) -> None:
        ...
        prompt = (
            ...
            f"\n1. 开场（1-2个动作）"
            f"\n2. 主体用loop覆盖大部分时长，每个loop体内放3-5个不同动作"
            f"\n3. 中间可换一组loop体现变化"
            f"\n4. 结尾（1-2个动作）"
            f"\n总输出控制在15个标签以内。"
            f"\n示例（{duration_s}秒歌曲）："
            ...
            f"<reachy_mini:dance name=\"groovy_sway_and_roll\"/>"
            f"<reachy_mini:antennas_move left=\"0.3\" right=\"-0.3\"/>"
            f"<reachy_mini:dance name=\"simple_nod\"/>"
            f"</loop>"
            f"<reachy_mini:emotion name=\"excited\"/>"
            f"<loop times=\"{max(1, dance_count // 3)}\">"
            f"<reachy_mini:dance name=\"side_to_side_sway\"/>"
            f"<reachy_mini:head_move roll=\"0.15\" pitch=\"0.1\" yaw=\"0.0\"/>"
            f"<reachy_mini:dance name=\"yeah_nod\"/>"
            f"</loop>"
